...
#    X2 = pd.DataFrame(adata_Vars2.X.toarray()[:, ], index=adata_Vars2.obs.index, columns=adata_Vars2.var.index)
    ...
    if type is 'ATAC':
        ...
        # print(f"G_gp_df.shape[0]*2: {G_gp_df.shape[0]*2}")
        # # print(f"protein_value: {protein_value}")
        # print(f"G_gp_df['Gene']: {len(G_gp_df['Gene'])}")
        # print(f"G_gp_df['Peak']: {len(G_gp_df['Peak'])}")
        # print(f"adata_Vars1.n_vars+adata_Vars2.n_vars: {adata_Vars1.n_vars+adata_Vars2.n_vars}")
        # print(f"adata_Vars1.n_vars: {adata_Vars1.n_vars}")
        # print(f"adata_Vars2.n_vars: {adata_Vars2.n_vars}")
